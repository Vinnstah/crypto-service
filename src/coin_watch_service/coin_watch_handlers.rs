...
use super::{coin_watch_client::CoinWatchClient, models::{AggregatedCoinInformation, ListOfCoinsRequest}};
use crate::{
    coin_watch_service::models::{Coin, CoinMeta, CoinMetaRequest},
    state::AppState,
};
...
pub async fn get_aggregated_coin_list(
    ...
    for coin in &list_of_coins.1.0 {
        ...
        for (idx, coin) in list_of_coins.1.0.iter().enumerate() {
            list_of_aggregated_coins.push(AggregatedCoinInformation { name: coin.code.clone(), symbol: coin.code.clone(), rank: 1.into(), rate: coin.rate, color: "1".into(), png64: coin_images[idx].clone() })
        };
